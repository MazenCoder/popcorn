...
Future<T?> showCupertinoModalBottomSheet<T>({
  ...
  final result =
      await Navigator.of(context, rootNavigator: useRootNavigator).push(CupertinoModalBottomSheetRoute<T>(
        builder: builder,
...
class CupertinoModalBottomSheetRoute<T> extends ModalBottomSheetRouteApp<T> {
  final Radius topRadius;
...
class CupertinoScaffoldInheirted extends InheritedWidget {
  ...
  const CupertinoScaffoldInheirted({Key? key,
    this.animation,
    required child,
    this.topRadius,
